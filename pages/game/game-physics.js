...
class PhysicsWorld {
  constructor(W, H) {
    ...
    this.overTimer = 0;
    this._id = 1;
    this.spawnLevel = this._pickSpawn();
    this.aimX = W / 2;
    this._merges = [];
  }
  ...
  nextSpawn() { this.spawnLevel = this._pickSpawn(); }
  ...
  drop(x) {
    ...
    this.balls.push({ id: this._id++, x: cx, y: r + 2, vx: 0, vy: 0, level: lv, merging: false });
    this.nextSpawn();
  }
  ...
  _resolveMerges() {
    ...
    for (const [a, c] of this._merges) {
      ...
      this.balls.push({ id: this._id++, x: nx, y: ny, vx: 0, vy: 0, level: nl, merging: false });
      this.score += LEVELS[nl].score;
